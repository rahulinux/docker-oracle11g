...
RUN test -f /etc/yum.repos.d/libselinux.repo && mv /etc/yum.repos.d/libselinux.repo /etc/yum.repos.d/libselinux.repo.disabled
...
# install UEK kernel
RUN yum install -y elfutils-libs gcc dracut
...
# create directories && disable mlock limit 
RUN mkdir /opt/oracle /opt/oraInventory /opt/datafile \
 && chown oracle:oinstall -R /opt && sed -i '/memlock/s/^/#/g' /etc/security/limits.conf
...
# Add scripts
RUN rpm -i http://dl.fedoraproject.org/pub/epel/6/x86_64//pwgen-2.07-1.el6.x86_64.rpm
